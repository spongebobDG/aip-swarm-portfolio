...
FrontierSearch::FrontierSearch(nav2_costmap_2d::Costmap2D* costmap,
  ...
  , goal_continuity_scale_(goal_continuity_scale)
...
{
  double base_cost = (potential_scale_ * frontier.min_distance *
                      costmap_->getResolution()) -
                     (gain_scale_ * frontier.size * costmap_->getResolution());

  if (goal_continuity_scale_ <= 0.0) {
    return base_cost;
  }

  // 이전 목표와의 거리 패널티: 같은 구역 인근 frontier 우선 탐색
  double dx = frontier.centroid.x - prev_goal_.x;
  double dy = frontier.centroid.y - prev_goal_.y;
  double dist_from_goal = std::sqrt(dx * dx + dy * dy);
  return base_cost + goal_continuity_scale_ * dist_from_goal;
}
}  // namespace frontier_exploration
